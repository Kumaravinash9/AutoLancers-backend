...
from app.services.bidding import BiddingError, check_availability, submit_bid_for_job
...
def make_job(**overrides) -> Job:
    defaults = dict(
        ...
        rejected=False,
        status="drafted",
        external_bid_id=None,
    )
    defaults.update(overrides)
    return Job(**defaults)


def make_token(scope: str | None) -> OAuthToken:
    return OAuthToken(user_id=1, platform="freelancer", access_token="x", scope=scope)
...
class TestGates:
    ...
    async def test_disabled_install_blocks(self, settings, no_network):
        ...
        with pytest.raises(BiddingError, match="switched off"):
            await submit_bid_for_job(_bidder_session(), 1, make_job(), 500, 7, confirm=True)
    ...
    async def test_unconfirmed_request_blocks(self, settings, no_network):
        ...
        with pytest.raises(BiddingError, match="not confirmed"):
            await submit_bid_for_job(_bidder_session(), 1, make_job(), 500, 7, confirm=False)
        assert not no_network["client"]
    ...
    async def test_nonsense_terms_block(self, settings, no_network, amount, period, match):
        ...
        with pytest.raises(BiddingError, match=match):
            await submit_bid_for_job(_bidder_session(), 1, make_job(), amount, period, confirm=True)
        assert not no_network["client"]
...
class TestScopeIsEnforcedOnTheWritePath:
    ...
    async def test_read_only_token_never_reaches_freelancer(self, settings, no_network):
        ...
        with pytest.raises(BiddingError, match="read-only"):
            await submit_bid_for_job(session, 1, make_job(), 500, 7, confirm=True)
        assert not no_network["client"]
...
class TestRecording:
    async def test_success_records_the_bid(self, settings, monkeypatch):
        ...
        bid_id = await submit_bid_for_job(session, 1, job, 750.0, 10, confirm=True)

        assert bid_id == "55501"
        assert job.external_bid_id == "55501"
        assert job.bid_amount == 750.0
        assert job.bid_period_days == 10
        assert job.status == "submitted"
        assert isinstance(job.bid_submitted_at, dt.datetime)
    ...
    async def test_a_recorded_bid_cannot_be_repeated(self, settings, no_network):
        ...
        with pytest.raises(BiddingError, match="Already bid"):
            await submit_bid_for_job(_bidder_session(), 1, job, 750.0, 10, confirm=True)
